public class Level1
{
   public static String PatternUnlock(int N, int [] hits) {
        ...
        double[] linesArray = CheckArray(hits);
        double sumLength = LengthOfLines(linesArray);
        return FormatLengthOfString(sumLength);
    }

    public static String FormatLengthOfString(double lengthOfLine) {
        String formatstring;
        //Округляем до 5 цифр
        DecimalFormat df = new DecimalFormat("#####.#####");
        formatstring = df.format(lengthOfLine);
    ...
    public static double LengthOfLines(double[] lines) {
        double lengthOfLines = 0.0;
        for (double v : lines) {
            lengthOfLines += + v;
        }
